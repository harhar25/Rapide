-- Fix Polymorphic FK on parts_requests
-- The 'requested_by' column can be a technician_id OR a personnel_id (job_controller)
-- SQL FKs cannot point to multiple tables based on a 'role' column.
-- We must remove the FK constraint on 'requested_by' to allow technician IDs.
...
CREATE TABLE parts_requests_new (
    ...
    -- Removed FK on requested_by to allow polymorphic IDs
);
